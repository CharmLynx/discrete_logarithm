...
__int128_t crt(const vector<__int128_t>& a, const vector<__int128_t>& m) {
    __int128_t M = 1;
    for (__int128_t mod : m) M *= mod;

    __int128_t x = 0;
    for (size_t i = 0; i < a.size(); ++i) {
        __int128_t Mi = M / m[i];
        __int128_t inv = mod_inv(Mi, m[i]);
        x = (x + a[i] * inv % M * Mi % M) % M;
    }
    return x;
}
...
__int128_t silver_polig_hellman(__int128_t a, __int128_t b, __int128_t n) {
    ...
    vector<__int128_t> remainders, moduli;
    ...
    for (auto& [p, l] : factorization) {
        ...
        __int128_t alpha = mod_pow(a, e, n); // α^e

        for (int i = 0; i < l; ++i) {
            __int128_t exp = int_pow(p, l - i - 1);
            __int128_t t = mod_pow(be, exp, n);

            // Пошук j такого, що (α^e)^j ≡ t (mod n)
            int d = -1;
...
int main() {
    ...
    duration<double> sph_time = end - start;

    cout << fixed << setprecision(4);
    cout << "SPH x = "; print128(x); cout << endl;
    cout << "SPH time avg: " << sph_time.count() / 5 << " s" << endl;
    ...
    duration<double> bf_time = end2 - start2;

    cout << "Brute x = "; print128(x1); cout << endl;
    cout << "Brute time avg: " << bf_time.count() / 5 << " s" << endl;
